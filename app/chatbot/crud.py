...
def create_history_message(db: Session, history_message: HistoryMessageCreate):
    db_history = HistoryMessage(
        ...
        history_id = history_message.history_id,
    )
    db.add(db_history)
    db.commit()
...
def get_history_id_by_session_id(db: Session, session_id):
    history_db = db.query(HistoryMessage.history_id).filter(HistoryMessage.session_id == session_id).first()
    return history_db