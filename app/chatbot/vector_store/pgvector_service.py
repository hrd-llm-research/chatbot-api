...
class PgvectorService:
    ...
    def get_collections(self) -> list:
        with self.engine.connect() as connection:
            try:
                query = text("SELECT * FROM pulic.langchain_pg_collection")
    ...
    def delete_collection_service(self, collection_name):
        ...
        try:
            with self.engine.connect() as connection:
                ...
                # query = text(f"Delete From langchain_pg_collection where name = :name")
                # connection.execute(query, {'name':collection_name})
                connection.commit()
